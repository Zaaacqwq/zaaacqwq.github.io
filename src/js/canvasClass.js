class Point {
  ...
  update(ParticlePolymerizeFlag = true, options, mx, my) {
    ...
    //移动速度
    this.spx = (this.nx - this.x) / (ParticlePolymerizeFlag ? 60 : 60);
    this.spy = (this.ny - this.y) / (ParticlePolymerizeFlag ? 60 : 60);
...
class DameDaneParticle {
  ...
  updateCenter(newX, newY) {
    this.renderX = newX;
    this.renderY = newY;
    this._InitParticle(this._imgArr, true); // Re-init particle positions
  }
}
